...
const getComments = async (req = request, res = response) => {
  ...
  try {
    const comments = await Comment.find().sort("-date").limit(limit).skip(from);
      ...
      if (commentUpdated) {
        return res.status(200).json({
          ...
          msg:
            "El comentario de: " +
            commentUpdated.name +
            ", ha sido aprobado correctamente",
        });
      } else {
        res.status(500).json({
          ok: false,
...
const getCommentsById = async (req, res) => {
  ...
  if (isValidObjectId(id)) {
    const comment = await Comment.find({ for_id: id });
    ...
    return res.status(400).json({
      ok: false,
      msg: "No se pudieron obtener los comentarios de este ID ",
    });
  } else {
    return res.status(400).json({
      ok: false,
      msg: "Este identificador no es válido",
    });
  }
};
